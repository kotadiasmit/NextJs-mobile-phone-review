...
import { addReview, updateReview } from "../utils/apis";

const ReviewForm = ({ initialState }) => {
  const [reviewObj, setReviewObj] = useState(initialState);
  const isUpdate = Object.keys(reviewObj).includes("_id");
  ...
  const submitForm = async (event) => {
    ...
    const response = isUpdate
      ? await updateReview(reviewObj, reviewObj._id)
  ...
  return (
    <div className="min-h-[calc(100vh-56px)] w-100 p-5">
      <h1 className="text-center font-bold mb-10 text-2xl text-slate-500 dark:text-slate-400">
        {isUpdate ? "Update" : "Add"} your Mobile Phone&apos;s valuable review
      </h1>
      <form
      ...
      >
        ...
        <button className="mt-5 font-semibold border rounded-md px-2 py-1 text-white bg-blue-800 active:bg-blue-600">
          {!isUpdate ? "Add Review" : "Update Review"}
        </button>
        {successMsg.msg && (
          <p
